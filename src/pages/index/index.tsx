...
const HomePage: FC = () => {
  ...
  const handleContactTeacher = (teacher: TeacherItem) => {
    if (!isLoggedIn) {
      Taro.showModal({ title: '提示', content: '请先登录后再联系教师', confirmText: '去登录', success: (res) => { if (res.confirm) goToLogin() } })
      return
    }
  ...
  const teacherEntries = [
    ...
    { icon: <Share2 size={22} color="#F59E0B" />, text: '转发赚钱', bgColor: 'bg-amber-100', action: () => Taro.showToast({ title: '功能开发中', icon: 'none' }) },
    { icon: <Wallet size={22} color="#2563EB" />, text: '收益中心', bgColor: 'bg-blue-100', action: () => Taro.navigateTo({ url: '/pages/earnings/index' }) },
  ]

  const parentEntries = [
    { icon: <Search size={22} color="#2563EB" />, text: '找教师', bgColor: 'bg-blue-100', action: () => Taro.navigateTo({ url: '/pages/teacher/list' }) },
    { icon: <Building2 size={22} color="#10B981" />, text: '找机构', bgColor: 'bg-green-100', action: goToOrgList },
    { icon: <Plus size={22} color="#F59E0B" />, text: '发布需求', bgColor: 'bg-amber-100', action: goToPublishDemand },
    ...
    <View className="min-h-screen bg-gray-100">
      ...
      <View className="flex flex-row items-center justify-between px-4 py-2 bg-white border-b border-gray-100">
        ...
        <View className="flex flex-row items-center bg-blue-50 px-2 py-1 rounded-full" onClick={handleSwitchRole}>
          <Text className="block text-sm text-blue-600 mr-1">{currentRole === 0 ? '家长端' : '教师端'}</Text>
          <ChevronDown size={14} color="#2563EB" />
        </View>
      </View>
      ...
      <ScrollView scrollY className="h-screen box-border">
        ...
        <View className="bg-white mb-2">
          <View className="flex flex-row items-center justify-between px-4 py-3">
            <Text className="block text-base font-semibold text-gray-900">{currentRole === 1 ? '附近需求' : '附近教师'}</Text>
          ...
          {currentRole === 0 && (loading || listLoading ? (
            ...
            <View className="py-10 text-center">
              <Text className="block text-sm text-gray-400">暂无附近教师</Text>
            </View>
          ))}
            ...
            <>
              <View className="flex flex-row items-center justify-between mx-4 mb-2 p-3 bg-blue-50 rounded-xl" onClick={goToPublishDemand}>
                <View className="flex flex-col">
                  ...
                  <Text className="block text-xs text-gray-500">让优质教师主动联系您</Text>
                </View>
                <ChevronRight size={20} color="#9CA3AF" />
              </View>
