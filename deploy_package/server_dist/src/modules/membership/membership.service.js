...
let MembershipService = class MembershipService {
    async getMembershipPlans(role) {
        ...
        if (error)
            throw new Error(`查询会员套餐失败: ${error.message}`);
        return data;
    }
    async buyMembership(userId, planId) {
        ...
        const { data: plan, error: planError } = await client
            ...
            .maybeSingle();
        if (planError)
            throw new Error(`查询套餐失败: ${planError.message}`);
        if (!plan)
    ...
    async handlePaymentSuccess(paymentNo, transactionId) {
        const client = (0, supabase_client_1.getSupabaseClient)();
        const { data: payment, error: paymentError } = await client
            .from('payments')
            .select(`
      ...
      `)
            ...
            .maybeSingle();
        if (paymentError)
            throw new Error(`查询支付记录失败: ${paymentError.message}`);
        if (!payment)
        ...
        const { error: userError } = await client
            ...
            .eq('id', userId);
        if (userError)
            throw new Error(`更新会员状态失败: ${userError.message}`);
    ...
    async triggerCommission(userId, paymentId, amount) {
        const client = (0, supabase_client_1.getSupabaseClient)();
    ...
    async getUserMembership(userId) {
        ...
        const isActive = user?.membership_type === 1 &&
