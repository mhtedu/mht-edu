...
let RobotService = class RobotService {
    ...
    async handleMessage(userId, message, conversationId) {
    ...
    async getWelcomeMessage(targetRole) {
    ...
    matchResponse(message, isMember, role) {
    ...
    getSuggestActions(isMember, role) {
        const actions = [];
        if (!isMember) {
            actions.push({ text: '开通会员', action: 'membership' });
        }
        if (role === 0) {
            actions.push({ text: '如何找老师', action: '教师' }, { text: '发布需求', action: '订单' });
        }
        else if (role === 1) {
            actions.push({ text: '查看需求', action: '订单' }, { text: '如何抢单', action: '抢单' });
        }
        return actions;
    }
    async saveMessage(conversationId, userId, userMessage, robotResponse) {
        try {
            await executeQuery(`
...
exports.RobotService = RobotService = __decorate([
